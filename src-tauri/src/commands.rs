...
pub struct AppConfig {
    ...
    /// 数据存储目录（可选，默认为应用数据目录）
    #[serde(default, alias = "data_directory")]
...
pub async fn get_data_directory(app_handle: tauri::AppHandle) -> Result<String, String> {
    ...
    if let Some(ref custom_dir) = config.data_directory {
        // 如果配置中指定了自定义目录，返回该目录
        Ok(custom_dir.clone())
    } else {
        // 否则返回默认的应用数据目录
...
#[tauri::command]
pub async fn set_data_directory(path: String, app_handle: tauri::AppHandle) -> Result<(), String> {
    // 验证路径是否存在且可写
    ...
    // 更新配置
    let mut config = app_config::get_config().await
        .map_err(|e| format!("Failed to read config: {}", e))?;
    
    config.data_directory = Some(path.clone());
    
    app_config::update_config(config, app_handle).await
